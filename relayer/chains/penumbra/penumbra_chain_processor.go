...
func (pcp *PenumbraChainProcessor) queryCycle(ctx context.Context, persistence *queryCyclePersistence) error {
	...
	// don't want to cause PenumbraChainProcessor to quit here, can retry again next cycle.
	if err != nil {
		pcp.log.Error(
			"Failed to query latest height after max attempts",
			zap.Uint("attempts", latestHeightQueryRetries),
	...
	for i := persistence.latestQueriedBlock + 1; i <= persistence.latestHeight; i++ {
		...
		eg.Go(func() (err error) {
			...
			params["height"] = i
			_, err = pcp.chainProvider.RPCCaller.Call(queryCtx, "block_results", params, blockRes)
			return err
		})
		eg.Go(func() (err error) {
			queryCtx, cancelQueryCtx := context.WithTimeout(ctx, queryTimeout)
			defer cancelQueryCtx()
			ibcHeader, err = pcp.chainProvider.IBCHeaderAtHeight(queryCtx, i)
			return err
		})
