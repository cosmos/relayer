...
func (cp *CosmosProvider) MakeEncodingConfig() params.EncodingConfig {
	...
	marshaler := cp.NewProtoCodec(interfaceRegistry, cp.Config.AccountPrefix)
...
func (cp *CosmosProvider) UseSDKContext() func() {
	...
	sdkConf.SetBech32PrefixForAccount(cp.Config.AccountPrefix, cp.Config.AccountPrefix+"pub")
	sdkConf.SetBech32PrefixForValidator(cp.Config.AccountPrefix+"valoper", cp.Config.AccountPrefix+"valoperpub")
	sdkConf.SetBech32PrefixForConsensusNode(cp.Config.AccountPrefix+"valcons", cp.Config.AccountPrefix+"valconspub")
...
func (cp *CosmosProvider) NewProtoCodec(interfaceRegistry types.InterfaceRegistry, accountPrefix string) *ProtoCodec {
	return &ProtoCodec{interfaceRegistry: interfaceRegistry, useContext: cp.UseSDKContext}
}
