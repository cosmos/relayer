...
func (c *Chain) InjectTrustedFields(dst *Chain, header *tmclient.Header) (*tmclient.Header, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get trusted header, please ensure header at the height %d has not been pruned by the connected node: %w", h.TrustedHeight.RevisionHeight, err)
	}
