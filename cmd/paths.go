	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, c := range srcClients {
				...
				if clnt.GetLatestHeight() != 0 && !clnt.IsFrozen() { // TODO: re-enable this check && clnt.GetChainID() == dst {
			...
			for _, c := range dstClients {
				// TODO: support other client types through a switch here as they become available
				clnt, _ := clientTypes.UnpackClientState(c.ClientState)
				if clnt.GetLatestHeight() != 0 && !clnt.IsFrozen() { // TODO: re-enable this check && clnt.GetChainID() == src {
